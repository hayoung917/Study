...
void handle_add(char* name)
{
    ...
    add(strdup(name), (char*)(strlen(number) > 0 ? strdup(number) : strdup(empty)),
        (char*)(strlen(email) > 0 ? strdup(email) : strdup(empty)),
        (char*)(strlen(group) > 0 ? strdup(group) : strdup(empty)));
}
